...
\paragraph{Deployed memory with retained retrieval vectors.}
The joint payload examples above describe the semantic representation. In the integrated prototype, dense navigation still retains the 1,536-byte FP32 retrieval vector per item. At 5M items and 100k concepts, reusing those vectors with FP32 heads costs 7,834.8\,MB, whereas retaining the same vectors and adding Binary1 item codes and heads costs 7,981.6\,MB (decimal MB; excluding graph/container overhead and small shared transform metadata). Thus the compressed semantic representation does not imply a reduction in this deployment's total memory. A table of eight calibrated FP32 concept logits adds only 32 bytes/item; its cost grows as $4NC$ and merits an explicit small-vocabulary comparison. A PQ64 int4 head can persist in 212 bytes/concept while still requiring a 65,548-byte active LUT. Its quality and timing remain follow-up measurements, not established results.
...
\paragraph{Status of the controlled follow-up.}
The historical timing values above come from the completed earlier harness. The new \path{notebooks/ras_review_experiments_colab.ipynb} uses one custom traversal for live, materialized, and over-fetch methods, shuffles measurement order, repeats timings, and sweeps $ef$ independently of the over-fetch multiplier. It adds a materialized per-concept logit table whose composition work is timed, alongside the precomposed-score control. The extracted graph, dependency lockfile, per-query timings, and environment metadata are saved for replay. These new measurements are pending; they must be assessed before updating the historical predicate-cost or matched-recall conclusions.
